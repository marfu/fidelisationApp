...
public class SentinelMenu extends AbstractMenu implements Widget,ComponentSystemEventListener {
    ...
    public void processEvent(ComponentSystemEvent event) throws AbortProcessingException {
        if(event instanceof PostAddToViewEvent) {
            ...
            String version = RequestContext.getCurrentInstance().getApplicationContext().getConfig().getBuildVersion();
            int majorVersion = Integer.parseInt(version.substring(0,1));
            String[] resources = majorVersion >= 6 ? MODERN_RESOURCES : LEGACY_RESOURCES;
